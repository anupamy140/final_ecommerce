// src/components/shared/BrandLogos.tsx
    ...
    return (
        <div className="py-12 border-t border-gray-100 dark:border-gray-800">
            <div className="max-w-5xl mx-auto flex justify-around items-center gap-8 flex-wrap">
                {logos.map((logo, i) => (<img key={i} src={logo} alt={`Brand ${i}`} className="h-8 object-contain dark:invert dark:brightness-0 opacity-60" />))}
            </div>
        </div>
    );
};