...
const ImageZoom: React.FC<ImageZoomProps> = ({ src, alt }) => {
    ...
    return (
        <div className="relative w-full aspect-square" onMouseEnter={() => !isLoading && setShowZoom(true)} onMouseLeave={() => setShowZoom(false)} onMouseMove={handleMouseMove}>
            <AnimatePresence>
                {isLoading && <motion.div initial={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-gray-100 dark:bg-gray-800 animate-pulse rounded-xl" />}
            </AnimatePresence>
            <img 
                ref={imgRef} 
                src={src} 
