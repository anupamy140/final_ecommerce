...
// ======================================================================
// ... (API Wrapper code remains the same)
const api = {
...
// --- 3. CUSTOM HOOKS ---
// ======================================================================
// ... (Custom Hooks code remains the same)
...
// --- 4. UI SUB-COMPONENTS ---
// ======================================================================
// ... (Button, Dialog, Sheet, ImageWithLoader, ImageZoom, TextType components remain the same)
...
// --- START: Ballpit Animation Component ---
// This is the full code for the Ballpit animation you provided.
// It includes all necessary Three.js classes and logic.
// Component inspired by Kevin Levron: https://x.com/soju22/status/1858925191671271801
// ... (All classes from the prompt: X, W, Y, Z, pointer helpers, etc. are pasted here)
interface XConfig {
...
const Header = () => {
  ...
  const handleProfileClick = () => { setProfileDropdownOpen(false); navigate('/profile'); };
              ...
              <div className="relative">
                ...
                {profileDropdownOpen && (
                  <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="absolute right-0 top-full mt-2 w-48 bg-white dark:bg-gray-900 border dark:border-gray-700 rounded-lg shadow-lg p-1.5">
                      ...
                      <button onClick={handleProfileClick} className="w-full flex items-center gap-2 text-left px-3 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-md transition-colors"><User size={16} /> My Profile</button>
                      <button onClick={api.logout} className="w-full flex items-center gap-2 text-left px-3 py-2 text-sm text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-md transition-colors"><LogOut size={16} /> Logout</button>
                  </motion.div>
                )}
                </AnimatePresence>
              </div>
        ...
        <div className="flex gap-5 group mx-auto">
          <div onClick={() => onCategorySelect("")} className={`flex-shrink-0 w-60 cursor-pointer transition-all duration-500 ease-in-out ${selectedCategory && selectedCategory !== "" ? 'opacity-50 scale-95' : 'opacity-100 scale-100'} group-hover:opacity-50 group-hover:scale-95 hover:!opacity-100 hover:!scale-105 hover:-translate-y-4`}>
              <div className={`relative h-[28rem] w-full bg-gray-200 overflow-hidden transition-all duration-300 rounded-2xl ${selectedCategory === "" || selectedCategory === null ? 'ring-4 ring-gray-900 dark:ring-yellow-400 ring-offset-4 ring-offset-gray-50 dark:ring-offset-gray-950' : ''}`}>
                  <ImageWithLoader src="https://images.pexels.com/photos/1926769/pexels-photo-1926769.jpeg?auto=compress&cs=tinysrgb" alt="All Products" className="w-full h-full" />
          ...
          {categories.map((category) => (
            <div key={category} onClick={() => onCategorySelect(category)} className={`flex-shrink-0 w-60 cursor-pointer transition-all duration-500 ease-in-out ${selectedCategory && selectedCategory !== category ? 'opacity-50 scale-95' : 'opacity-100 scale-100'} group-hover:opacity-50 group-hover:scale-95 hover:!opacity-100 hover:!scale-105 hover:-translate-y-4`}>
              <div className={`relative h-[28rem] w-full bg-gray-200 overflow-hidden transition-all duration-300 rounded-2xl ${selectedCategory === category ? 'ring-4 ring-gray-900 dark:ring-yellow-400 ring-offset-4 ring-offset-gray-50 dark:ring-offset-gray-950' : ''}`}>
                ...
                console.log(`/public/${category}.jpg`);
                <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent"></div>
...
const ProfilePage = () => {
    const { user, setWishlistOpen } = useApp();
    const navigate = useNavigate();

    const menuItems = [
        { icon: User, label: "Personal Information" },
        { icon: Package, label: "My Orders" },
        { icon: Heart, label: "My Wishlist", action: () => setWishlistOpen(true) },
    ...
    return (
        <div className="max-w-4xl mx-auto py-12 px-4">
            ...
            <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl p-8">
                <div className="flex items-center gap-6 pb-6 border-b dark:border-gray-800">
                    <div className="w-20 h-20 flex items-center justify-center bg-gray-900 dark:bg-gray-50 text-white dark:text-gray-900 font-bold rounded-full text-4xl">
                        {user?.trim().charAt(0).toUpperCase()}
                    </div>
                    <div>
                        <h2 className="text-2xl font-bold dark:text-white">{user}</h2>
                        <p className="text-md text-gray-500 dark:text-gray-400">Welcome back!</p>
                    </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8">
                    {menuItems.map(item => (
                        <button key={item.label} onClick={item.action} className="w-full flex items-center gap-4 p-4 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800/50 transition-colors text-left">
                            <item.icon className="w-6 h-6 text-gray-500 dark:text-gray-400" />
                            <span className="font-semibold text-lg text-gray-800 dark:text-gray-200">{item.label}</span>
                        </button>
                    ))}
                </div>
                <div className="mt-8 pt-6 border-t dark:border-gray-800">
                    <Button onClick={api.logout} variant="destructive" className="w-full md:w-auto flex items-center gap-2">
...
// --- START: New Success Page Component ---
const SuccessPage = () => {
    const { fetchCart } = useApp();
    
    // When this page loads, the cart should be empty.
    // We call fetchCart() to sync the state with the server.
    useEffect(() => {
        fetchCart();
    }, [fetchCart]);
...
// --- END: New Success Page Component ---

// --- START: New Cancel Page Component ---
const CancelPage = () => {
    return (
...
// --- END: New Cancel Page Component ---

// ======================================================================
// --- 6. NEW LAYOUT COMPONENT (Manages Global State) ---
// ======================================================================
...
const Layout = () => {
    ...
    // --- MODIFIED: Updated checkout function with new success/cancel URLs ---
...
const router = createHashRouter([
    {
        ...
        children: [
            ...
            {
                path: "success", // New route for payment success
                element: <SuccessPage />
            },
            {
                path: "cancel", // New route for payment cancellation
                element: <CancelPage />
            }
        ]
    }
